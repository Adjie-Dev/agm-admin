...
return new class extends Migration
{
    ...
    {
        Schema::create('puja_pagi', function (Blueprint $table) {
            $table->id();
            $table->integer('urutan')->unique()->comment('Urutan section puja pagi');
            $table->string('judul', 150);
            $table->string('durasi', 20)->nullable()->comment('Otomatis dari audio: MM:SS');
            $table->text('deskripsi')->nullable();
            $table->string('audio_path')->nullable()->comment('Path file: audio/puja-pagi/nama.mp3');
