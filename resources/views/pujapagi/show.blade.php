...
@section('title', $pujaPagi->judul)

@section('header', 'Detail Puja Pagi')
        ...
        <div class="p-8">
            ...
            <div class="mb-6">
                <div class="flex items-center space-x-3 mb-4">
                    <div class="flex items-center justify-center w-12 h-12 bg-blue-500/10 rounded-lg">
                        <span class="text-blue-400 font-bold text-xl">{{ $pujaPagi->urutan }}</span>
                    </div>
                    <h1 class="text-3xl font-bold text-white">{{ $pujaPagi->judul }}</h1>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-4 text-sm text-gray-400">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <span>{{ $pujaPagi->created_at->format('d F Y') }}</span>
                    </div>
            ...
            <div class="mb-6">
                ...
                <div class="bg-slate-900/50 rounded-xl p-6 border border-slate-700/50">
                    <p class="text-gray-300 leading-relaxed">{{ $pujaPagi->deskripsi }}</p>
                </div>
            </div>
            @endif
                    ...
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-400">{{ $pujaPagi->audio_ukuran_formatted }}</p>
                            </div>
                        </div>
                    </div>
            ...
            <!-- Teks Pali -->
            @if($pujaPagi->teks_pali)
            <div class="mb-6">
                <h3 class="text-lg font-bold text-white mb-3">Teks Pali</h3>
                <div class="bg-slate-900/50 rounded-xl p-6 border border-slate-700/50">
                    <p class="text-gray-300 leading-relaxed whitespace-pre-wrap font-mono">{{ $pujaPagi->teks_pali }}</p>
                </div>
            </div>
            @endif

            <!-- Terjemahan -->
            @if($pujaPagi->terjemahan)
            <div class="mb-6">
                <h3 class="text-lg font-bold text-white mb-3">Terjemahan</h3>
                <div class="bg-slate-900/50 rounded-xl p-6 border border-slate-700/50">
                    <p class="text-gray-300 leading-relaxed whitespace-pre-wrap">{{ $pujaPagi->terjemahan }}</p>
                </div>
            </div>
            @endif
            ...
            <div class="flex flex-wrap items-center justify-end gap-4">
                <a href="{{ route('puja-pagi.edit', $pujaPagi) }}"
                ...
                <form action="{{ route('puja-pagi.destroy', $pujaPagi) }}" method="POST" class="inline-block">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
