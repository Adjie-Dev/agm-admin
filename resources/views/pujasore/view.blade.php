...
@section('title', $pujaSore->judul)

@section('header', 'Detail Puja Sore')
        ...
        <div class="p-8">
            ...
            <div class="mb-6">
                <div class="flex items-center space-x-3 mb-4">
                    <div class="flex items-center justify-center w-12 h-12 bg-blue-500/10 rounded-lg">
                        <span class="text-blue-400 font-bold text-xl">{{ $pujaSore->urutan }}</span>
                    </div>
                    <h1 class="text-3xl font-bold text-white">{{ $pujaSore->judul }}</h1>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-4 text-sm text-gray-400">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <span>{{ $pujaSore->created_at->format('d F Y') }}</span>
                    </div>
            ...
            <div class="mb-6">
                ...
                <div class="bg-slate-900/50 rounded-xl p-6 border border-slate-700/50">
                    <p class="text-gray-300 leading-relaxed">{{ $pujaSore->deskripsi }}</p>
                </div>
            </div>
            @endif

            <!-- Audio Player -->
            @if($pujaSore->audio_path)
            <div class="mb-6">
                ...
                <div class="bg-slate-900/50 rounded-xl p-6 border border-slate-700/50">
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <p class="text-white font-medium">{{ $pujaSore->audio_nama_asli }}</p>
                                <p class="text-sm text-gray-400">{{ $pujaSore->audio_ukuran_formatted }}</p>
                            </div>
                        </div>
                    </div>
                    <audio controls class="w-full">
                        <source src="{{ asset('storage/' . $pujaSore->audio_path) }}" type="{{ $pujaSore->audio_mime }}">
            ...
            <!-- Teks Pali -->
            @if($pujaSore->teks_pali)
            <div class="mb-6">
                <h3 class="text-lg font-bold text-white mb-3">Teks Pali</h3>
                <div class="bg-slate-900/50 rounded-xl p-6 border border-slate-700/50">
                    <p class="text-gray-300 leading-relaxed whitespace-pre-wrap font-mono">{{ $pujaSore->teks_pali }}</p>
                </div>
            </div>
            @endif

            <!-- Terjemahan -->
            @if($pujaSore->terjemahan)
            <div class="mb-6">
                <h3 class="text-lg font-bold text-white mb-3">Terjemahan</h3>
                <div class="bg-slate-900/50 rounded-xl p-6 border border-slate-700/50">
                    <p class="text-gray-300 leading-relaxed whitespace-pre-wrap">{{ $pujaSore->terjemahan }}</p>
                </div>
            </div>
            @endif
            ...
            <div class="flex flex-wrap items-center justify-end gap-4">
                <a href="{{ route('puja-sore.edit', $pujaSore) }}"
                ...
                <form action="{{ route('puja-sore.destroy', $pujaSore) }}" method="POST" class="inline-block">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
