...
@section('title', 'Amituojing')

@section('header', 'Amituojing')
...
<div class="max-w-7xl mx-auto">
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-white mb-2">Daftar Amituojing</h2>
            <p class="text-gray-400">Kelola konten audio Amituojing</p>
        </div>
        <a href="{{ route('amituojing.create') }}"
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-slate-700/50">
                    @forelse($amituojings as $amituojing)
                    <tr class="hover:bg-slate-700/30 transition-colors duration-200">
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center w-10 h-10 bg-blue-500/10 rounded-lg">
                                <span class="text-blue-400 font-bold text-lg">{{ $amituojing->urutan }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="text-white font-medium">{{ $amituojing->judul }}</div>
                            @if($amituojing->deskripsi)
                            <div class="text-sm text-gray-400 mt-1">{{ Str::limit($amituojing->deskripsi, 50) }}</div>
                            @endif
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <div class="text-sm">
                                    <div class="text-gray-300">{{ $amituojing->audio_ukuran_formatted }}</div>
                                </div>
                            </div>
                            @else
                            <span
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end space-x-2">
                                <a href="{{ route('amituojing.show', $amituojing) }}"
                                ...
                                <form action="{{ route('amituojing.destroy', $amituojing) }}" method="POST"
                                    class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center justify-center space-y-3">
                                ...
                                <p class="text-gray-400 text-lg">Belum ada data Amituojing</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
